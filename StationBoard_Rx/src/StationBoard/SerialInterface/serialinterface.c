/*** StationBoard - SerialInterface module implementation
  ** Warthog Robotics
...
void serialInterface_configureUART() {
    ConfigIntUART1(UART_RX_INT_DIS & UART_RX_INT_PR5 & UART_TX_INT_DIS);
}
...
void serialInterface_initializeUART() {
    ...
    brgFloat = ((STATIONBOARD_Fcy*1000.0) / (4.0*serialInterface_baudRate)) - 1.0;
...
void __attribute__((__interrupt__,no_auto_psv)) _DMA0Interrupt() {   
    ...
    uint8 i;
    for (i = 0; i < 5; i++) {
        player[i] = serialInterface_rxBuffer[i];
    }
    for (i = 0; i < 26; i++) {
        command[i] = serialInterface_rxBuffer[i+5];
    }
